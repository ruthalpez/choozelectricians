...
const LIVE_API_URL = process.env.NEXT_PUBLIC_API_URL + "/api/electricians";
const DEV_API_URL = "http://localhost:8000/api/electricians";
...
export const getByStateCity = async (
  ...
  try {
    ...
    const response = await apiClient.get(
      `/${state}/${city}/electricians-contractors`,
      { params },
    );
    ...
    console.error("Error fetching electricians data:", error);
    return null;
  }
};
