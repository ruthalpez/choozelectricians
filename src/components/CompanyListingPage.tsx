...
const CompanyListingPage = () => {
  ...
  const isValidRoute = useMemo(() => {
    return state && city && category === "electricians-contractors";
  }, [state, city, category]);
  ...
  const findZipCodeMatch = useCallback(
    (zipCode: string) => {
      ...
      if (match) {
        ...
        return {
          url: `/${stateSlug}/${citySlug}/electricians-contractors`,
          city: match.city,
          state: match.state,
          zip: match.zip,
        };
              ...
              {faqs.map((faq) => (
                <div key={faq.id} className="space-y-4 about-text">
                  <h2 className="text-[26px] font-bold font-poppins">
                    Q&A Based On What Customers Are Saying About Electricians
